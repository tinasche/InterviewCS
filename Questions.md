...
3. What is an interface? A form of abstract class with **only** abstract members i.e., no method definition or property instantiation. It enforces a contract for the classes which *implement* the interface to define the members described therein.
...
The CTS defines how types are created, used and managed in the CLR
...
<br/>An abstract method is a method that uses the *abstract* keyword to describe its method signature. It is contained within an abstract class and must be implemented by a class which inherits from it.

7. What is boxing and unboxing? The conversion of a type from value to reference is called boxing. The conversion of a type from reference to value is called unboxing. Boxing converts a type from an instance of a value type to that of a reference type. Unboxing is the revers process.

8. What is a delegate?
Delegates are reference types that behave like function pointers in C++. They are used for event handling and callback functions. A delegate is linked to the method/object. 
...
Reflection is a mechanism that provides objects (of type Type) which describe assmeblies, modules, and types. Reflection allows to access attributes. 
...
18. Demonstrate your knowledge of deconstruction of types. What are discards?
Deconstruction is a technique to assign or extract multiple values from some variable returned by some expression/operation.
```
// Using Tuple type to group multiple data elements in a lightweight data structure.
(T, T) spec1 = (30, 1.76);
Console.WriteLine($"My age is {spec1.Item1}, height: {spec1.Item2}");

// Supply field names is possible as well!
(T Age, T Height) spec2 = (30, 1.76);
...
A *record* is a reference type with built-in functionality for data encapsulation. Records come in 2 variants, a reference type (with *record class*) and a value type (with *record struct*) as definition keywords. You can create records with immutable properties via positional parameters or standard property syntax. Use the with operator to clone and edit properties in a new record.
```
// Declare reference- and value- type record.
// Using positional parameters and standard property syntax.
record class/struct Pet(string name, string species, double weight);
record class/struct Pet 
{ 
    string name {get;set;}
    string species {get;set;}
    ouble weight{get;set;}
};

// Sample code demonstrating record type
Meal meal1 = new Meal("thumped peas", 14.5, true);
Meal meal2 = meal1 with { name = "truffled peas" };
Meal meal3 = new Meal(isLight:true, name: "sadza nemuriwo", calories: 3.4);

var result = (meal1 == meal2) ? "they are equivalent" : "they are not equivalent";
var result2 = (meal1.Equals(meal2)) ? "they are equal" : "they are not equal";
var (name, weight, isLight) = meal2;

Console.WriteLine(result);
Console.WriteLine(result2);
Console.WriteLine(meal3.GetType());
Console.WriteLine(meal3);
Console.WriteLine($"{name} has a weight contribution of {weight} so is considereded a {(isLight ? "light meal" : "heavy meal")}");

record class Meal(string name, double calories, bool isLight);
```

21. What is Serialization in C#?
Serialization is the process of converting an object into a stream of bytes to store the object or trasmit it to memory, a database or a file. Searilization allows the developer to save the state of an object or re-create it providing storage of objects or data exchange. With searialization a developer can perform the following actions:
- Data exchange with remote application via web service
- Passing an object between domains
- Passing an object through a firewall as a JSON or XML string
- Maintain security or user-specific information across applications
Json serialization is handled by System.Text.Json
XML serialization is handled by System.Xml.Serialization
```
// Have an object to represent the state.
const string filename = @"./car.json";
Car myCar = new Car();
if (File.Exists(filename))
{
    var carsFromJson = File.ReadAllText(filename);
    myCar = JsonSerializer.Deserialize<Car>(carsFromJson);
    Console.Write(myCar);
}
```
